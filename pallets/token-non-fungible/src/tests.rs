...
fn mint_should_not_work() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			TokenNonFungible::mint(Origin::signed(ALICE), 1, ALICE, 1),
			Error::<Test>::InvalidId
		);
		assert_ok!(TokenNonFungible::create_token(
			Origin::signed(ALICE),
		...
		assert_noop!(
			TokenNonFungible::mint(Origin::signed(BOB), 1, ALICE, 100),
			Error::<Test>::NoPermission
		);
		assert_ok!(TokenNonFungible::mint(Origin::signed(ALICE), 1, ALICE, 2));
		assert_noop!(
			TokenNonFungible::mint(Origin::signed(ALICE), 1, ALICE, 2),
			Error::<Test>::TokenAlreadyMinted
		);
	})
}
...
fn approve_should_not_work() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			TokenNonFungible::approve(Origin::signed(ALICE), 1, BOB, 50),
			Error::<Test>::NotFound
		);
		assert_ok!(TokenNonFungible::create_token(
			Origin::signed(ALICE),
		...
		assert_ok!(TokenNonFungible::mint(Origin::signed(ALICE), 1, ALICE, 0));
		assert_noop!(
			TokenNonFungible::approve(Origin::signed(ALICE), 1, BOB, 1),
			Error::<Test>::NotFound
		);
		assert_noop!(
			TokenNonFungible::approve(Origin::signed(ALICE), 1, ALICE, 0),
...
fn transfer_from_should_not_work() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			TokenNonFungible::transfer_from(Origin::signed(BOB), 1, ALICE, CHARLIE, 0),
			Error::<Test>::TokenNonExistent
		);
		...
		assert_ok!(TokenNonFungible::approve(Origin::signed(ALICE), 1, BOB, 0));
		assert_noop!(
			TokenNonFungible::transfer_from(Origin::signed(BOB), 1, ALICE, BOB, 1),
			Error::<Test>::NotOwnerOrApproved
		);

		assert_noop!(
			TokenNonFungible::transfer_from(Origin::signed(BOB), 1, BOB, ALICE, 1),
			Error::<Test>::NotOwnerOrApproved
		);

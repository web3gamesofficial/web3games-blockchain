...
fn test_swap_currency_to_token_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ExchangeNft::swap_currency_to_token(origin,id,token_ids,token_amounts_out,max_currency));
    })
}
...
fn test_swap_token_to_currency_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ExchangeNft::swap_token_to_currency(origin,id,token_ids,token_amounts_in,min_currency));
    })
}
...
//         assert_eq!(TokenFungible::balance_of(1,0),0);
//
//         assert_ok!(ExchangeNft::remove_liquidity(origin,id,token_ids,liquidities,min_currencies,min_tokens));
//     })
// }

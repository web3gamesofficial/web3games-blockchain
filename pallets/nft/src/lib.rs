...
pub mod pallet {
	...
	pub(super) type Collections<T: Config> = StorageMap<
		...
		Blake2_128Concat,
		CollectionId,
		CollectionInfo<T::AccountId, BalanceOf<T>>
	>;
	
	#[pallet::storage]
	#[pallet::getter(fn next_collection_id)]
	pub(super) type NextCollectionId<T: Config> = StorageValue<
		_,
		CollectionId,
		ValueQuery
	>;
	...
	pub(super) type NextAssetId<T: Config> = StorageMap<
		_,
		Blake2_128Concat,
		CollectionId,
		AssetId,
	...
	impl<T:Config> Pallet<T> {
		...
		pub fn create_collection(origin: OriginFor<T>, properties: Vec<u8>) -> DispatchResultWithPostInfo {
...
impl<T: Config> Pallet<T> {
	...
	pub fn do_mint(
		...
		NextAssetId::<T>::try_mutate(collection_id, |id| -> Result<Vec<AssetId>, DispatchError> {
			...
			*id = id
			...
			Self::deposit_event(Event::TokenMint(collection_id, asset_ids.clone(), to.clone()));
			Ok(asset_ids)
		})
	}
	...
	pub fn do_transfer_from(
		...
		NftAssets::<T>::try_mutate(collection_id, asset_id, |asset_info| -> DispatchResult {
			...
			Self::deposit_event(Event::TokenTransferred(collection_id, asset_id, from.clone(), to.clone()));
	...
	pub fn do_destroy_collection(
		...
		Collections::<T>::try_mutate_exists(collection_id, |collection_info| -> DispatchResult {
			let info = collection_info.take().ok_or(Error::<T>::CollectionNotFound)?;
			ensure!(info.owner == *who, Error::<T>::NoPermission);
			ensure!(info.total_supply == Zero::zero(), Error::<T>::CannotDestroyCollection);

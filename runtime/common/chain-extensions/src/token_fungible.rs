		...
		<E::T as SysConfig>::AccountId: UncheckedFrom<<E::T as SysConfig>::Hash> + AsRef<[u8]>,
	{
		match func_id {
			...
			65537 => {
				...
				let (fungible_token_id,name, symbol, decimals): (<C as pallet_token_fungible::Config>::FungibleTokenId,Vec<u8>, Vec<u8>, u8) =
					env.read_as_unbounded(env.in_len())?;
				env.charge_weight(10000)?;

				let id = pallet_token_fungible::Pallet::<E::T>::do_create_token(
					&caller, fungible_token_id,name, symbol, decimals,
				)?;
